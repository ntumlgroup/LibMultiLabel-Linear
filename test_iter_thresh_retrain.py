...
if __name__ == "__main__":
    ...
    for d in datasets:
        ...
        if d == "amazoncat-13k/ver1":
            d = "amazoncat-13k"
        threshold_experiment.create_dirs(f"runs/iter-thresh-retrain/{d}")
        ...
        model, metrics_collection, eval_time = pruning.iter_thresh_retrain(
            ...
            log_path=f"runs/iter-thresh-retrain/{d}/logs/{d}-run-logs.txt",
            K=100,
            dmax=10,
        ...
        with open(f"runs/iter-thresh-retrain/{d}/train_times.txt", "a") as file:
            file.write(f"{datetime.datetime.now()} {d}: {str(end-start)} \n")
        with open(f"runs/iter-thresh-retrain/{d}/eval_times.txt", "a") as file:
            file.write(f"{datetime.datetime.now()} {d}: {str(eval_time)} \n")

        linear.utils.save_pipeline_threshold_experiment(
            f"runs/iter-thresh-retrain/{d}/models",
            preprocessor,
            model,
        ...
        libmultilabel.common_utils.dump_log(
            ...
            split="test",
            log_path=f"runs/iter-thresh-retrain/{d}/logs/iter-thresh-retrain.json",
        )
