...
_threshold () {
    for d in "${dataset[@]}"; do
        ...
        python threshold_experiment.py --threshold_options "$thresholdoptions" \
                                        --model_path "$ROOT/$d/$reg/$dirsetting" \
    ...
    for d in "${dataset[@]}"; do
        ...
        do  
            ...
            python experiment_utils.py --function_name "thresh_node_predict" \
                            --dataset_name "$d" \
                            --model_path "$filename" \
                            --log_path "$ROOT/$d/$reg/$dirsetting/logs" \
            ...
            echo "${name} | $(($end-$start)) seconds" >> "$ROOT/$d/$reg/$dirsetting/prediction-time-logs.txt"
        done
    done
...
dataformat="svm"
thresholdtechnique="thresh_node"
seed="1337"
reg="L2"
...
# thresholdoptions="lin/0/100/.01"
ROOT="/mnt/HDD-Seagate-16TB-2/justinchanchan8/tree-prunning-results/runs/tree"
dirsetting="85-100-geometric-node-tr-val-split"
# dirsetting="0-100-linear-node-tr-val-split"
cd ..
